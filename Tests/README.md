...
| `HistoryTests` | schema v2 (Core's demographic model (Core 0.5.0)): a v1 file migrates in place with data intact; the built-in vocabulary seeds every factor and sector and never overwrites a Def row; region and cohort years, factor levels, flows, births and settlements upsert idempotently; person events are a save's history along its lineage with branches excluded; a collected commit takes its history with it |
